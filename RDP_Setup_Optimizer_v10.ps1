<#
.SYNOPSIS
    RDP Setup & Optimizer v10.0 - Silk Edition
.DESCRIPTION
    Zero-flicker smooth UI with working live terminal.
    Phase 1: System Optimization (25 steps, UI-FIRST reversed)
    Phase 2: Software Installation (22 steps, FAST mode)
    Works via: irm https://raw.githubusercontent.com/.../RDP_Setup_Optimizer_v10.ps1 | iex
.VERSION
    10.0
#>
...
function LogIt {
    ...
    if($script:Logs.Count-gt 80){$script:Logs=$script:Logs[-80..-1]}
}
...
function SmartDraw {
    ...
    $currentHash="$PhaseNum|$PhaseName|$done|$fail|$run|$pct|$et|"
    if($script:CurrentTask){$currentHash+="$($script:CurrentTask.N)|$($script:CurrentTask.S)"}
    $currentHash+="|TL:$($script:TermLogs.Count)|LG:$($script:Logs.Count)"

    # Only clear if hash changed significantly
    if($currentHash -ne $script:LastDrawHash){
        Clear-Host
        $script:LastDrawHash=$currentHash
    } else {
        # Just return cursor to top without clearing
    ...
    WC "╔══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════╗" "BGU"
    BoxLine "" "BGU"
    BoxLine "  $($script:C.G1)🚀$($script:C.X)  $($script:C.NC)$($script:C.B)RDP Setup & Optimizer v10.0 - Silk Edition$($script:C.X)  $($script:C.GY)[ Phase $PhaseNum : $PhaseName ]$($script:C.X)" "BGU"
    BoxLine "" "BGU"
    WC "╚══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════╝" "BGU"
    WC ""
    ...
    # === TASK LIST (Compact - last 8 only to save space) ===
    WC "┌──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┐" "NC"
    ...
    # === LIVE TERMINAL BOX ===
    WC "┌──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┐" "NC"
    WC "│$($script:C.B)$($script:C.NG)  🔴 LIVE TERMINAL  (Real-time output)                                                                               $($script:C.X)│" "NC"
    WC "├──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┤" "NC"
    ...
    WC "├──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┤" "NC"
    $vl=$script:Logs|Select-Object -Last 4
    if($vl){
        foreach($l in $vl){
    ...
    for($i=$vl.Count;$i-lt 4;$i++){BoxLine "" "NC"}
    WC "└──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┘" "NC"
    WC ""
    WC "  $($script:C.GY)💡 Tip: Live terminal shows real-time output. Do not close this window.$($script:C.X)" "GY" -N
    WC "                              $($script:C.LG)✅ Zero-flicker smooth UI | Windows Server 2025 RDP$($script:C.X)" "LG"
}

# === FAST WINGET INSTALLER WITH LIVE OUTPUT ===
function Install-WingetFast {
    param([string]$Id,[string]$Description="",[string]$ExtraArgs="")
    TermLog ">>> Installing: $Description"

    $argsStr = "install --id `"$Id`" -e --silent --accept-package-agreements --accept-source-agreements --disable-interactivity"
    if($ExtraArgs) { $argsStr += " $ExtraArgs" }
    TermLog "> winget $argsStr"
    ...
    try {
        ...
        # Read output line by line for live terminal
        while(-not $proc.StandardOutput.EndOfStream) {
            $line = $proc.StandardOutput.ReadLine()
            if($line -and $line.Trim()) {
                TermLog "  $line"
                # Redraw to show live output
                SmartDraw -Tasks $script:P2Tasks -PhaseName $script:PhaseName -PhaseNum $script:PhaseNum
            }
        }
        ...
        if($errOut) {
            $errLines = $errOut -split "`r?`n" | Where-Object { $_.Trim() }
            foreach($el in $errLines | Select-Object -Last 3) {
                TermLog "  ! $el"
            }
        }
        ...
        if($proc.ExitCode -eq 0 -or $proc.ExitCode -eq -1978335189 -or $proc.ExitCode -eq 3010) {
            TermLog "[OK] $Description installed"
            return $true
        } else {
...
function RunStep {
    ...
    SmartDraw -Tasks $TaskList -PhaseName $script:PhaseName -PhaseNum $script:PhaseNum
...
function Run-Phase1 {
    ...
    RunStep 5 {
        ...
        TermLog ">>> Hibernation off"
        powercfg -h off | Out-Null
        TermLog "[OK] Hibernation off"
        return $true
    } $t
    ...
    RunStep 9 {
        ...
        TermLog ">>> Telemetry off"
        Stop-Service DiagTrack -Force -ErrorAction SilentlyContinue
        Set-Service DiagTrack -StartupType Disabled
        TermLog "[OK] Telemetry off"
        return $true
    } $t
    ...
    RunStep 11 {
        ...
        TermLog ">>> DeliveryOpt off"
        Stop-Service DoSvc -Force -ErrorAction SilentlyContinue
        Set-Service DoSvc -StartupType Disabled
        TermLog "[OK] DeliveryOpt off"
        return $true
    } $t
    ...
    RunStep 17 {
        ...
        TermLog ">>> Explorer to ThisPC"
        Set-ItemProperty -Path "HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Advanced" -Name "LaunchTo" -Value 1
        TermLog "[OK] Explorer done"
        return $true
    } $t
    ...
    RunStep 18 {
        ...
        TermLog ">>> Extensions on"
        Set-ItemProperty -Path "HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced" -Name "HideFileExt" -Value 0
        TermLog "[OK] Extensions on"
        return $true
    } $t
    ...
    RunStep 22 {
        ...
        TermLog ">>> Animations off"
        Set-ItemProperty -Path "HKCU:\Control Panel\Desktop\WindowMetrics" -Name "MinAnimate" -Value 0
        TermLog "[OK] Animations off"
        return $true
    } $t
    ...
    RunStep 24 {
        ...
        if($guid){powercfg /setactive $guid|Out-Null;LogIt "GUID: $guid" "OK";TermLog "[OK] UltimatePerf enabled";return $true}
        throw "No GUID"
    } $t

    $script:CurrentTask=$null
    SmartDraw -Tasks $t -PhaseName $script:PhaseName -PhaseNum $script:PhaseNum
}
...
function Run-Phase2 {
    ...
    RunStep 6 {
        ...
        TermLog ">>> wsreset -i"
        try {
            Start-Process wsreset -ArgumentList "-i" -Wait -WindowStyle Hidden | Out-Null
            TermLog "[OK] Store reset"
        } catch { TermLog "[WARN] wsreset: $($_.Exception.Message)" }
        return $true
    } $t

    # S08 - Xbox Identity
    RunStep 7 {
        return (Install-WingetFast -Id "9WZDNCRD1HKW" -Description "Xbox Identity" -ExtraArgs "--source msstore")
    } $t
    ...
    RunStep 9 {
        ...
        TermLog ">>> wsreset -i (verify)"
        try {
            Start-Process wsreset -ArgumentList "-i" -Wait -WindowStyle Hidden | Out-Null
            TermLog "[OK] Store verified"
        } catch { TermLog "[WARN] wsreset: $($_.Exception.Message)" }
        return $true
    } $t
    ...
    # S11 - Node.js
    RunStep 10 {
        $ok = Install-WingetFast -Id "OpenJS.NodeJS.LTS" -Description "Node.js" -ExtraArgs "--scope machine"
        if(-not $ok) {
            TermLog "[Fallback] Chocolatey Node.js"
        ...
        $np="C:\Program Files\nodejs";$mp=[Environment]::GetEnvironmentVariable("Path","Machine")
        if($mp -notlike "*$np*"){[Environment]::SetEnvironmentVariable("Path",($mp.TrimEnd(";")+";"+$np),"Machine")}
    ...
    RunStep 11 {
        ...
        if(-not $ok) {
            TermLog "[Fallback] Chocolatey Git"
            [Net.ServicePointManager]::SecurityProtocol=[Net.SecurityProtocolType]::Tls12
            if(-not (Get-Command choco -ErrorAction SilentlyContinue)){
                Invoke-WebRequest "https://community.chocolatey.org/install.ps1" -UseBasicParsing | Invoke-Expression
            }
        ...
        $gp="C:\Program Files\Git\cmd";$mp=[Environment]::GetEnvironmentVariable("Path","Machine")
        if($mp -notlike "*$gp*"){[Environment]::SetEnvironmentVariable("Path",($mp.TrimEnd(";")+";"+$gp),"Machine")}
        $env:Path=[Environment]::GetEnvironmentVariable("Path","Machine")+";"+[Environment]::GetEnvironmentVariable("Path","User")
        return $true
    } $t

    # S13-S17 - Python versions
    $pyVersions = @(
        @{Ver="3.10";Path="C:\Program Files\Python310";Scripts="C:\Program Files\Python310\Scripts"},
        @{Ver="3.11";Path="C:\Program Files\Python311";Scripts="C:\Program Files\Python311\Scripts"},
        @{Ver="3.12";Path="C:\Program Files\Python312";Scripts="C:\Program Files\Python312\Scripts"},
        @{Ver="3.13";Path="C:\Program Files\Python313";Scripts="C:\Program Files\Python313\Scripts"},
        @{Ver="3.14";Path="C:\Program Files\Python314";Scripts="C:\Program Files\Python314\Scripts"}
    )

    for($i=0; $i -lt $pyVersions.Count; $i++) {
        $py = $pyVersions[$i]
        $idx = 12 + $i
        RunStep $idx {
            LogIt "Installing Python $($py.Ver)..." "INFO"
            $ok = Install-WingetFast -Id "Python.Python.$($py.Ver)" -Description "Python $($py.Ver)" -ExtraArgs '--scope machine --override "/quiet InstallAllUsers=1 PrependPath=1 Include_launcher=1 InstallLauncherAllUsers=1 Include_pip=1 Include_test=0"'
            if($ok) {
                $mp=[Environment]::GetEnvironmentVariable("Path","Machine")
                foreach($p in @($py.Path,$py.Scripts)){if($mp -notlike "*$p*"){$mp=$mp.TrimEnd(";")+";"+$p}}
                [Environment]::SetEnvironmentVariable("Path",$mp,"Machine")
                $env:Path=$mp+";"+[Environment]::GetEnvironmentVariable("Path","User")
            }
            return $ok
        } $t
    }
        ...
        foreach($v in $versions){
            ...
            try {
                ...
                if($LASTEXITCODE -eq 0){
                    TermLog "  Upgrading pip Python $v..."
                    ...
                    TermLog "  [OK] Python $v done"
                }
            } catch { TermLog "  [SKIP] Python $v" }
        }
        return $true
    } $t
    ...
    RunStep 18 {
        ...
        TermLog ">>> VS Code"
        # FIX: Single-quoted override prevents comma parsing error
        ...
        if($ok) {
            $cp="C:\Program Files\Microsoft VS Code\bin";$mp=[Environment]::GetEnvironmentVariable("Path","Machine")
            if(Test-Path $cp -and $mp -notlike "*$cp*"){[Environment]::SetEnvironmentVariable("Path",($mp.TrimEnd(";")+";"+$cp),"Machine")}
            $env:Path=[Environment]::GetEnvironmentVariable("Path","Machine")+";"+[Environment]::GetEnvironmentVariable("Path","User")
            TermLog "[OK] VS Code done"
        }
        return $ok
    } $t
    ...
    RunStep 19 {
        ...
        if($installed){TermLog "[OK] IDM done";return $true}
        return $false
    } $t
...
function Show-Summary {
    ...
    if($p1Fail-gt 0 -or $p2Fail-gt 0){
        WC "  $($script:C.LY)⚠️  Some steps failed:$($script:C.X)" "LY"
        $allFail=($P1+$P2)|Where-Object{$_.S-eq"Fail"}
        foreach($f in $allFail){WC "       $($script:C.LR)• $($f.N)$($script:C.X)" "LR"}
    }else{
        WC "  $($script:C.LG)🎉 Perfect run! All done!$($script:C.X)" "LG"
        WC "  $($script:C.SI)   Your RDP is fully optimized and loaded.$($script:C.X)" "SI"
    }

    $lf="$env:TEMP\RDP_Setup_Optimizer_v10_Log.txt"
    $script:Logs|ForEach-Object{"[{0}] [{1}] {2}"-f$_.T,$_.L,$_.M}|Out-File $lf -Encoding UTF8
    WC ""
    WC "  $($script:C.GY)💾 Log: $lf$($script:C.X)" "GY"
    WC ""
...
function Show-MainMenu {
    ...
    WC "    $($script:C.G3)╚═╝  ╚═╝╚═════╝ ╚═╝         ╚══════╝╚══════╝   ╚═╝     ╚═══╝  ╚═╝           ╚═══╝   ╚═════╝     ╚══════╝ ╚═════╝$($script:C.X)" "G3"
    WC ""
    WC "                              $($script:C.B)$($script:C.NC)Windows Server 2025 - RDP Setup & Optimizer v10.0$($script:C.X)" "NC"
    WC "                                       $($script:C.GY)Silk Edition | Zero Flicker | Live Terminal | UI-First$($script:C.X)" "GY"
    WC ""
    WC "  $($script:C.LY)⚠️  Requires Administrator. Modifies system settings.$($script:C.X)" "LY"
    WC "  $($script:C.SI)   Phase 1: UI-First reversed | Phase 2: FAST winget with live output$($script:C.X)" "SI"
    WC ""
    WC "  ┌──────────────────────────────────────────────────────────────────────────────────────────────────────────────────┐" "NC"
    ...
    WC "  └──────────────────────────────────────────────────────────────────────────────────────────────────────────────────┘" "NC"
    WC ""
    WC "  $($script:C.GY)GitHub: irm https://raw.githubusercontent.com/abubakarmalikgit/RDP-Setup/main/RDP_Setup_Optimizer_v10.ps1 | iex$($script:C.X)" "GY"
    WC ""
}
...
function Start-RDPSetupOptimizer {
    ...
    if(-not $pr.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)){
        WC "";WC "  $($script:C.LR)❌ ERROR: Run as Administrator!$($script:C.X)" "LR";WC ""
        return
    }

    [Console]::OutputEncoding=[System.Text.Encoding]::UTF8
    $Host.UI.RawUI.WindowTitle="RDP Setup & Optimizer v10.0 - Silk"
    ...
    switch($choice){
        ...
        "4"{WC "";WC "  $($script:C.LY)👋 Exiting.$($script:C.X)" "LY";WC ""}
        default{WC "";WC "  $($script:C.LR)❌ Invalid choice.$($script:C.X)" "LR";WC ""}
    }
}
