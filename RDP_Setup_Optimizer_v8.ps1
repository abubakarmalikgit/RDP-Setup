<#
.SYNOPSIS
    RDP Setup & Optimizer v8.0 - Ultimate Edition
.DESCRIPTION
    Phase 1: System Optimization (25 steps)
    Phase 2: Software Installation (22 steps)
    Works via: irm https://raw.githubusercontent.com/.../RDP_Setup_Optimizer_v8.ps1 | iex
    Dynamic ANSI via [char]27 - survives irm | iex corruption.
.VERSION
    8.0
#>
...
function WC {
    param([string]$T = "", [string]$C = "W", [switch]$N)
    $cc = switch($C) {
        ...
        "BR" { $script:C.BR } "BGU" { $script:C.BGU } "BGP" { $script:C.BGP }
        "BGC" { $script:C.BGC } "BGK" { $script:C.BGK }
        default { $script:C.W }
    }
...
function Line { param([string]$C = "NC", [int]$L = 120)
    WC ($script:C.B + ("═" * $L) + $script:C.X) $C
}

function BoxLine { param([string]$T = "", [string]$C = "NC", [int]$W = 118)
...
# === PHASE 1 TASKS (25) ===
$script:P1Tasks = @(
    @{ID=1;  N="Enable Ultimate Performance";     I="⚡"; S="Wait"; Desc="Activating Ultimate Performance power plan..."},
    @{ID=2;  N="Disable Visual Effects";            I="🎨"; S="Wait"; Desc="Stripping visual effects for raw performance..."},
    @{ID=3;  N="Disable Window Animations";         I="🪟"; S="Wait"; Desc="Killing window animations..."},
    @{ID=4;  N="Disable Transparency";              I="🔍"; S="Wait"; Desc="Removing transparency effects..."},
    @{ID=5;  N="Disable Startup Delay";             I="⏱️"; S="Wait"; Desc="Eliminating startup delay..."},
    @{ID=6;  N="Set Menu Delay to 0";               I="📋"; S="Wait"; Desc="Setting menu delay to zero..."},
    @{ID=7;  N="Show File Extensions";              I="📁"; S="Wait"; Desc="Revealing file extensions..."},
    @{ID=8;  N="Open Explorer to This PC";          I="💻"; S="Wait"; Desc="Configuring Explorer to This PC..."},
    @{ID=9;  N="Disable Game DVR";                  I="🎮"; S="Wait"; Desc="Disabling Game DVR overhead..."},
    @{ID=10; N="Disable Background Apps";           I="🚫"; S="Wait"; Desc="Killing background app access..."},
    @{ID=11; N="Optimize Multimedia";               I="🎵"; S="Wait"; Desc="Tuning multimedia system profile..."},
    @{ID=12; N="Disable SysMain";                   I="🗂️"; S="Wait"; Desc="Stopping SysMain (SuperFetch)..."},
    @{ID=13; N="Disable Windows Search";            I="🔎"; S="Wait"; Desc="Stopping Windows Search indexer..."},
    @{ID=14; N="Disable Delivery Optimization";     I="📦"; S="Wait"; Desc="Stopping Delivery Optimization..."},
    @{ID=15; N="Disable Error Reporting";           I="⚠️"; S="Wait"; Desc="Stopping Error Reporting service..."},
    @{ID=16; N="Disable Telemetry";                 I="📡"; S="Wait"; Desc="Stopping DiagTrack telemetry..."},
    @{ID=17; N="Disable Windows Update";            I="🔄"; S="Wait"; Desc="Configuring Windows Update policy..."},
    @{ID=18; N="Disable Consumer Features";         I="🏪"; S="Wait"; Desc="Disabling Windows Consumer Features..."},
    @{ID=19; N="Disable Windows Tips";              I="💡"; S="Wait"; Desc="Killing Windows tips and suggestions..."},
    @{ID=20; N="Disable Hibernation";               I="💤"; S="Wait"; Desc="Disabling system hibernation..."},
    @{ID=21; N="Enable Classic Context Menu";       I="🖱️"; S="Wait"; Desc="Restoring Windows 10 classic context menu..."},
    @{ID=22; N="Restart Explorer";                  I="🔄"; S="Wait"; Desc="Restarting Windows Explorer..."},
    @{ID=23; N="Apply Glow Theme";                  I="✨"; S="Wait"; Desc="Applying dark glow theme..."},
    @{ID=24; N="Show Desktop Icons";                I="🖥️"; S="Wait"; Desc="Configuring desktop icon visibility..."},
    @{ID=25; N="Taskbar Left + Search Box";         I="📌"; S="Wait"; Desc="Moving taskbar left and enabling search..."}
)
...
$script:P2Tasks = @(
    ...
    @{ID=7;  N="Microsoft Store Reset";             I="🏪"; S="Wait"; Desc="Resetting Microsoft Store..."},
...
function DrawDashboard {
    ...
    WC "╔══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════╗" "BGU"
    BoxLine "" "BGU"
    BoxLine "  $($script:C.G1)🚀$($script:C.X)  $($script:C.NC)$($script:C.B)RDP Setup & Optimizer v8.0 - Ultimate Edition$($script:C.X)  $($script:C.GY)  [ Phase $PhaseNum : $PhaseName ]$($script:C.X)" "BGU"
    BoxLine "" "BGU"
    WC "╚══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════╝" "BGU"
    WC ""
    ...
    if($script:CurrentTask) {
        $tc = switch($script:CurrentTask.S) {
            "Run"  { $script:C.LY }
            "Done" { $script:C.LG }
            "Fail" { $script:C.LR }
            default { $script:C.GY }
        }
        $ti = switch($script:CurrentTask.S) {
            "Run"  { "⚙️" }
            "Done" { "✅" }
            "Fail" { "❌" }
            default { "⏳" }
    ...
    # === LOGS ===
    WC "┌──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┐" "NC"
    WC "│$($script:C.B)$($script:C.NC)  LIVE LOG                                                                                                              $($script:C.X)│" "NC"
    WC "├──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┤" "NC"
    $vl = $script:Logs | Select-Object -Last 8
    if($vl) {
        foreach($l in $vl) {
            $co = switch($l.L) {
                "OK"   { $script:C.LG }
    ...
    for($i = $vl.Count; $i -lt 8; $i++) { BoxLine "" "NC" }
    WC "└──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┘" "NC"
    WC ""
    WC "  $($script:C.GY)💡 Tip: Do not close this window. The process is running in real-time.$($script:C.X)" "GY" -N
...
function RunStep {
    ...
    if($ok) {
        $task.S = "Done"
        LogIt "✅ DONE: $($task.N) in $($task.Dur)s" "OK"
    } else {
        $task.S = "Fail"
        LogIt "❌ FAIL: $($task.N) - $err" "ERR"
    }
    DrawDashboard -Tasks $TaskList -PhaseName $script:PhaseName -PhaseNum $script:PhaseNum
...
function Run-Phase1 {
    ...
    RunStep 0 {
        ...
        if($guid) { powercfg /setactive $guid | Out-Null; LogIt "PowerPlan GUID: $guid" "OK"; return $true }
...
function Run-Phase2 {
    ...
    RunStep 0 {
        ...
        winget install VideoLAN.VLC -e --silent --accept-package-agreements --accept-source-agreements | Out-Null
        return $?
    } $t
    ...
    RunStep 1 {
        ...
        winget install CodecGuide.K-LiteCodecPack.Full -e --silent --accept-package-agreements --accept-source-agreements | Out-Null
        return $?
    } $t
    ...
    RunStep 2 {
        ...
        winget install RARLab.WinRAR -e --silent --accept-package-agreements --accept-source-agreements | Out-Null
        return $?
    } $t
    ...
    RunStep 3 {
        ...
        winget install 7zip.7zip -e --silent --accept-package-agreements --accept-source-agreements | Out-Null
        return $?
    } $t
    ...
    RunStep 4 {
        ...
        winget install Valve.Steam -e --silent --accept-package-agreements --accept-source-agreements | Out-Null
        if(Test-Path "C:\Program Files (x86)\Steam\steam.exe") { Start-Process "C:\Program Files (x86)\Steam\steam.exe" }
        return $true
    } $t
    ...
    RunStep 5 {
        ...
        winget install --id 9MV0B5HZVK9Z --exact --source msstore --accept-package-agreements --accept-source-agreements | Out-Null
        Start-Process "xbox:"
        return $true
    } $t
    ...
    # S07 - MS Store Reset
    RunStep 6 {
        LogIt "Resetting Microsoft Store..." "INFO"
        wsreset -i | Out-Null
        return $true
    } $t
    ...
    RunStep 7 {
        ...
        winget install --id 9WZDNCRD1HKW --exact --source msstore --accept-package-agreements --accept-source-agreements | Out-Null
        return $?
    } $t
    ...
    RunStep 8 {
        ...
        winget install --id 9NZKPSTSNW4P --exact --source msstore --accept-package-agreements --accept-source-agreements | Out-Null
        return $?
    } $t
    ...
    RunStep 10 {
        ...
        if(Get-Command winget -ErrorAction SilentlyContinue) {
            winget install --id OpenJS.NodeJS.LTS -e --silent --scope machine --accept-package-agreements --accept-source-agreements | Out-Null
        } else {
    ...
    RunStep 11 {
        ...
        if(Get-Command winget -ErrorAction SilentlyContinue) {
            winget install --id Git.Git -e --silent --scope machine --accept-package-agreements --accept-source-agreements | Out-Null
        } else {
            [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
            if(-not (Get-Command choco -ErrorAction SilentlyContinue)) {
                Invoke-WebRequest "https://community.chocolatey.org/install.ps1" -UseBasicParsing | Invoke-Expression
    ...
    RunStep 12 {
        ...
        winget install --id Python.Python.3.10 -e --silent --scope machine --accept-package-agreements --accept-source-agreements --override "/quiet InstallAllUsers=1 PrependPath=1 Include_launcher=1 InstallLauncherAllUsers=1 Include_pip=1 Include_test=0" | Out-Null
    ...
    RunStep 13 {
        ...
        winget install --id Python.Python.3.11 -e --silent --scope machine --accept-package-agreements --accept-source-agreements --override "/quiet InstallAllUsers=1 PrependPath=1 Include_launcher=1 InstallLauncherAllUsers=1 Include_pip=1 Include_test=0" | Out-Null
    ...
    RunStep 14 {
        ...
        winget install --id Python.Python.3.12 -e --silent --scope machine --accept-package-agreements --accept-source-agreements --override "/quiet InstallAllUsers=1 PrependPath=1 Include_launcher=1 InstallLauncherAllUsers=1 Include_pip=1 Include_test=0" | Out-Null
    ...
    RunStep 15 {
        ...
        winget install --id Python.Python.3.13 -e --silent --scope machine --accept-package-agreements --accept-source-agreements --override "/quiet InstallAllUsers=1 PrependPath=1 Include_launcher=1 InstallLauncherAllUsers=1 Include_pip=1 Include_test=0" | Out-Null
    ...
    RunStep 16 {
        ...
        winget install --id Python.Python.3.14 -e --silent --scope machine --accept-package-agreements --accept-source-agreements --override "/quiet InstallAllUsers=1 PrependPath=1 Include_launcher=1 InstallLauncherAllUsers=1 Include_pip=1 Include_test=0" | Out-Null
    ...
    # S18 - Python Modules
    RunStep 17 {
        LogIt "Installing Python dev modules for all versions..." "INFO"
        ...
        foreach($v in $versions) {
            ...
            try {
                ...
                if($LASTEXITCODE -eq 0) {
                    LogIt "Upgrading pip for Python $v..." "INFO"
                    ...
                    LogIt "Python $v modules installed." "OK"
                }
            } catch { LogIt "Python $v not available, skipped." "WARN" }
        }
        return $true
    } $t
    ...
    RunStep 18 {
        ...
        winget install --id Microsoft.VisualStudioCode -e --silent --scope machine --accept-package-agreements --accept-source-agreements --override "/VERYSILENT /NORESTART /MERGETASKS=addcontextmenufiles,addcontextmenufolders,associatewithfiles,addtopath" | Out-Null
...
function Show-Summary {
    ...
    $lf = "$env:TEMP\RDP_Setup_Optimizer_v8_Log.txt"
...
function Show-MainMenu {
    Clear-Host
    WC ""
    WC "    $($script:C.G1)██████╗ ██████╗ ██████╗     ███████╗███████╗████████╗██╗   ██╗██████╗     $($script:C.G3)██╗   ██╗ ██████╗     ███████╗ ██████╗$($script:C.X)" "G1"
    WC "    $($script:C.G1)██╔══██╗██╔══██╗██╔══██╗    ██╔════╝██╔════╝╚══██╔══╝██║   ██║██╔══██╗    $($script:C.G3)██║   ██║██╔═══██╗    ██╔════╝██╔═══██╗$($script:C.X)" "G1"
    WC "    $($script:C.G2)██████╔╝██║  ██║██████╔╝    ███████╗█████╗     ██║   ██║   ██║██████╔╝    $($script:C.G4)██║   ██║██║   ██║    ███████╗██║   ██║$($script:C.X)" "G2"
    WC "    $($script:C.G2)██╔══██╗██║  ██║██╔═══╝     ╚════██║██╔══╝     ██║   ╚██╗ ██╔╝██╔═══╝     $($script:C.G4)╚██╗ ██╔╝██║   ██║    ╚════██║██║   ██║$($script:C.X)" "G2"
    WC "    $($script:C.G3)██║  ██║██████╔╝██║         ███████║███████╗   ██║    ╚████╔╝ ██║          $($script:C.G5)╚████╔╝ ╚██████╔╝    ███████║╚██████╔╝$($script:C.X)" "G3"
    WC "    $($script:C.G3)╚═╝  ╚═╝╚═════╝ ╚═╝         ╚══════╝╚══════╝   ╚═╝     ╚═══╝  ╚═╝           $($script:C.G5) ╚═══╝   ╚═════╝     ╚══════╝ ╚═════╝$($script:C.X)" "G3"
    WC ""
    WC "                              $($script:C.B)$($script:C.NC)Windows Server 2025 - RDP Setup & Optimizer v8.0$($script:C.X)" "NC"
    WC "                                       $($script:C.GY)Ultimate Performance + Dev Environment Setup$($script:C.X)" "GY"
    WC ""
    WC "  $($script:C.LY)⚠️  This script requires Administrator privileges and modifies system settings.$($script:C.X)" "LY"
    WC "  $($script:C.SI)   Designed for Windows Server 2025 RDP with 16GB+ RAM.$($script:C.X)" "SI"
    WC ""
    WC "  ┌──────────────────────────────────────────────────────────────────────────────────────────────────────────────────┐" "NC"
    WC "  │$($script:C.B)$($script:C.NC)  SELECT PHASE                                                                                                    $($script:C.X)│" "NC"
    WC "  ├──────────────────────────────────────────────────────────────────────────────────────────────────────────────────┤" "NC"
    WC "  │                                                                                                                  │" "NC"
    WC "  │   $($script:C.NG)[1]$($script:C.X)  $($script:C.B)Phase 1: System Optimization$($script:C.X)        $($script:C.GY)- 25 tweaks for ultimate RDP performance$($script:C.X)                    │" "NC"
    WC "  │                                                                                                                  │" "NC"
    WC "  │   $($script:C.NC)[2]$($script:C.X)  $($script:C.B)Phase 2: Software Installation$($script:C.X)    $($script:C.GY)- 22 apps: Steam, Python, VS Code, Node, Git, IDM...$($script:C.X)   │" "NC"
    WC "  │                                                                                                                  │" "NC"
    ...
    WC "  └──────────────────────────────────────────────────────────────────────────────────────────────────────────────────┘" "NC"
    WC ""
    WC "  $($script:C.GY)GitHub: irm https://raw.githubusercontent.com/abubakarmalikgit/RDP-Setup/main/RDP_Setup_Optimizer_v8.ps1 | iex$($script:C.X)" "GY"
    WC ""
}
...
function Start-RDPSetupOptimizer {
    ...
    if(-not $pr.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
        WC ""; WC "  $($script:C.LR)❌ ERROR: Administrator privileges required! Right-click PowerShell and select 'Run as Administrator'.$($script:C.X)" "LR"; WC ""
        return
    }

    [Console]::OutputEncoding = [System.Text.Encoding]::UTF8
    $Host.UI.RawUI.WindowTitle = "RDP Setup & Optimizer v8.0"
    ...
    switch($choice) {
        "1" {
            Run-Phase1
            Show-Summary -P1 $script:P1Tasks -P2 @()
        }
        "2" {
            Run-Phase2
            Show-Summary -P1 @() -P2 $script:P2Tasks
        }
        "3" {
            Run-Phase1
            Run-Phase2
            Show-Summary -P1 $script:P1Tasks -P2 $script:P2Tasks
        }
        "4" {
            WC ""; WC "  $($script:C.LY)👋 Exiting. No changes were made.$($script:C.X)" "LY"; WC ""
        }
        default {
            WC ""; WC "  $($script:C.LR)❌ Invalid choice. Exiting.$($script:C.X)" "LR"; WC ""
        }
    }
}
